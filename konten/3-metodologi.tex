...
\begin{algorithm}
    ...
    \If {$GPS\_Accuracy \geq Threshold$}
...
\begin{algorithm}
  ...
  \While {not reached final waypoint}
      ...
      \State $angle\_error \gets (angle\_error + \pi) \bmod (2\pi) - \pi$ \Comment{Normalize angle to [-\(\pi\), \(\pi\)]}


      \If {$perpendicular\_distance < threshold\_distance$ and $|angle\_error| < threshold\_angle$}
